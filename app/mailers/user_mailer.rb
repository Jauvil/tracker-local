class UserMailer < ActionMailer::Base
  ...
  def welcome_system_user(user, server_config)
    @user = user
    @server_config = server_config
    mail(from: get_support_email, to: @user.email, subject: "Welcome to #{@server_config.server_name}.") if @user.email.present?
  end

  def reset_user_password(user, school, server_config)
    @user = user
    @server_config = server_config
    if @user.school_id.present?
      @school_name = school.name
    ...
    mail(from: get_support_email, to: @user.email, subject: "Reset Password for #{@school_name} #{@server_config.server_name}.") if @user.email.present?
  end
