...
describe "School Listing", js:true do
  ...
  def has_nav_to_schools_page(school_assigned, show_subject_manager)
    ...
    def valid_edit_school
      ...
      assert_equal("/schools", current_path)
      page.find("a[data-url='/schools/#{@school1.id}/edit.js'] i.fa-edit", wait: 5).click
      page.should have_content("Edit School")
      ...
      within("#modal_popup .modal-dialog .modal-content .modal-body") do
        ...
        if (ServerConfig.first.try(:allow_subject_mgr) != true)
          # Egyptian Configuration
          page.should_not have_content("Allow Subject Managers")
          expect(page).to have_field('school[flag_pars][use_family_name]', checked: true)
          expect(page).to have_field('school[flag_pars][user_by_first_last]', checked: true)
          expect(page).to have_field('school[flag_pars][grade_in_subject_name]', checked: true)
          expect(page).to have_field('school[flag_pars][username_from_email]', checked: true)
        else
          ...
          expect(page).to have_field('school[flag_pars][username_from_email]', checked: false)
        end
      ...
      if (ServerConfig.first.try(:allow_subject_mgr) != true)
        page.should_not have_css('#allow-subject-mgr', text: "true")
        page.should_not have_content("Allow Subject Managers")
