...
describe "School Listing", js:true do
  ...
  it { has_valid_us_school_navigations(:teacher) }
  end
  ...
  it { has_valid_us_schools_summary(:school_administrator) }
  it { has_valid_us_school_navigations(:school_administrator) }
  end
  ...
  it { has_nav_to_us_schools_page(true) }
  it { has_valid_us_schools_summary(:researcher) }
  it { has_valid_us_school_navigations(:researcher) }
  end

  describe "as us researcher with no school selected" do
  before do
    @researcher = FactoryGirl.create :researcher
    sign_in(@researcher)
    @home_page = "/researchers/#{@researcher.id}"
  end
  it { has_nav_to_us_schools_page(false) }
  end
  ...
  it { has_nav_to_us_schools_page(true) }
  it { has_valid_us_schools_summary(:system_administrator)}
  it { has_valid_us_school_navigations(:system_administrator) }
  it { valid_edit_us_school }
  end

  describe "as us system administrator with no school selected" do
  before do
    @system_administrator = FactoryGirl.create :system_administrator
    sign_in(@system_administrator)
    @home_page = "/system_administrators/#{@system_administrator.id}"
  ...
  it { has_valid_us_school_navigations(:student) }
  end
  ...
  it { has_valid_us_school_navigations(:parent) }
  end
    ...
    if (role == :student || role == :parent)
      ...
      if (role == :teacher || role == :school_administrator || role == :system_administrator || role == :researcher)
        ...
        if (role == :teacher || role == :school_administrator)
          ...
          page.all("tr td.school-acronym").count.should == 6
          if (role == :system_administrator)
  ...
  def has_nav_to_us_schools_page(school_assigned)
    # only for system_administrators and researchers
    if !school_assigned
      # confirm school is not assigned
      within("#head-current-school") do
  ...
  def has_valid_us_schools_summary(role)
    # access to school listing and school summary page in header
    # to be run only for ('system_administrator' || 'researcher' || 'school_administrator')
  ...
  def has_valid_us_school_navigations(role)
